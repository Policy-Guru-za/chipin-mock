...
import { HOST_CREATE_PAYOUT_METHODS } from '@/lib/dream-boards/payout-methods';
...
const payoutSchema = z
  .object({
    ...
    payoutMethod: z.enum(HOST_CREATE_PAYOUT_METHODS),
...
const resolveBankAccountData = (
  rawBankAccountNumber: string | null,
  params: { encryptedFromDraft?: string; last4FromDraft?: string }
) => {
  if (!process.env.CARD_DATA_ENCRYPTION_KEY) {
    redirect('/create/payout?error=secure');
  }
  ...
  const bankAccountLast4 = rawBankAccountNumber
    ? rawBankAccountNumber.slice(-4)
    : params.last4FromDraft;
