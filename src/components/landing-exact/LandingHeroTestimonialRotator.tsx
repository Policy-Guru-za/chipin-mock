...
export function LandingHeroTestimonialRotator({ className }: LandingHeroTestimonialRotatorProps) {
  ...
  useEffect(() => {
    if (prefersReducedMotion || isPaused || landingTestimonials.length <= 1) {
      return;
    }
  ...
  const contentClassName = prefersReducedMotion ? undefined : 'animate-landing-testimonial-fade';
  ...
  return (
    ...
    >
      <div key={prefersReducedMotion ? 'static' : activeTestimonial} className={contentClassName}>
