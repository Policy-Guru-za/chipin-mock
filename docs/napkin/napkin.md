...
|------|--------|----------------|-------------------|
| 2026-03-16 | self | A user-uploaded branding asset arrived in the repo as a generic `Untitled.png`, and wiring runtime metadata directly to that temporary filename would have baked an accidental upload name into the app contract | When an uploaded asset becomes a runtime surface like the favicon, move it to a stable descriptive repo path first, then point metadata/contracts at that intentional path |
| 2026-03-16 | self | A homepage request wanted only the hero headline font to change, but the existing editorial token was shared with the testimonial quote ornament, so a token-level swap would have caused collateral typography drift | For one-off homepage font swaps, keep shared tokens stable and target the exact selector (or a dedicated headline-only token) instead of rewiring the global editorial token |
| 2026-03-16 | user | I interpreted a request to remove a small heading inside the homepage timeline section as permission to remove the entire heading stack, including the main `Gifting, together` title | For multi-level section headings, remove only the explicitly named tier and preserve the primary title unless the user clearly asks for the whole heading block to go |
| 2026-03-16 | self | I tried to preserve the homepage contributor constellation on narrower desktop widths even after the right rail became too narrow for its fixed scatter offsets, which turned a decorative desktop-only flourish into an overflow bug | For decorative hero constellations that rely on fixed absolute offsets, keep them only in the reviewed wide-desktop range; below the cutoff, remove them entirely and rebalance surrounding spacing instead of inventing a compact fallback |
| 2026-03-16 | self | I tried to satisfy the homepage desktop quote-stack layout by cloning testimonial/CTA/contributors into separate desktop and mobile render paths, which made the duplicate copy leak back into the DOM during real browser dogfood | For breakpoint-specific hero composition work, keep one semantic set of narrative/visual blocks in JSX and use shared rail wrappers plus CSS ordering/layout overrides to rearrange them across breakpoints instead of rendering duplicate desktop/mobile copies |
| 2026-03-16 | self | I kept trying to improve the desktop homepage quote/CTA spacing with seam and inset tuning even though those elements still lived in later sections below a tall first hero beat, so the whitespace was structural and could not be solved cleanly with margin tweaks alone | When desktop-only narrative elements need to sit directly beneath a hero stat chip, make that narrative rail explicit at the desktop breakpoint (and preserve mobile with shared reusable blocks) instead of relying on section spacing or pull-up hacks |
| 2026-03-16 | self | After removing a visible fixed-nav spacer, I let the homepage hero start exactly at the nav offset, which technically removed the strip but made the composition feel cramped against the nav border | After collapsing a fixed-nav spacer into the first visible section, add a small deliberate section-owned inset so the seam stays clean without sacrificing visual breathing room; lock that subtle-inset contract in a source-level regression test |
| 2026-03-16 | self | I tried to fix the homepage nav seam by tuning spacer heights while leaving the offset as a standalone blank block below a fixed nav; combined with start-of-hero spacing, that still reads as a white strip even when the math is technically correct | For fixed-nav homepage parity work, do not reserve nav clearance as a visible empty spacer. Hide the offset under the fixed nav itself or let the first visible section own it, then lock the no-strip contract with a source-level regression test |
| 2026-03-16 | self | A dev-only DOM-annotation tool like Agentation can look harmless enough to mount globally, but that would widen its DOM-reading blast radius across sensitive surfaces and make activation depend on whatever local dev server state happens to be running | For dev-only browser annotation tooling in this repo, isolate it in a dedicated client-only wrapper, mount it only on the intended page surface, and require both `NODE_ENV === 'development'` and an explicit public env flag so activation is deliberate and reversible |
| 2026-03-16 | self | I matched the homepage hero structure/copy to the reviewed source but left the desktop one-line headline expectation implicit, so the equal-column grid and current font scale were free to squeeze the text track and wrap the two intended hero lines | For homepage parity work, encode screenshot-level desktop text contracts explicitly: give the hero text track a guaranteed width, add desktop-only no-wrap where the design requires it, and add a regression test for that contract |
| 2026-03-16 | self | Ported the reviewed homepage nav seam into Tailwind `md/lg` spacer classes, which shifted the breakpoint contract from the prototype’s custom `1100px` tablet cutoff and created a visible white bar under the nav around 1024px widths | For homepage parity work, keep seam-critical spacing on one landing-specific CSS contract (shared custom properties/media queries) instead of approximating the reviewed breakpoint math with generic Tailwind breakpoints |
| 2026-03-16 | self | `docs:audit` crawled transient prototype markdown under `tmp/` and failed with a generic unclassified-file error, which obscured the real policy question; a future `CLAUDE.md` reintroduction would have failed the same vague way | Keep transient import/staging folders like `tmp/` and `.reference/` out of docs governance scope, and make retired root docs fail with explicit policy errors instead of generic missing-classification throws |
| 2026-03-16 | self | The homepage Hero diagnosis was harder because the app had overloaded `--font-display` to Fraunces and kept no dedicated editorial token, hiding that the reviewed prototype used DM Serif Display for shared display surfaces and Fraunces only for editorial Hero accents | Preserve the prototype font-token split in this repo: `--font-display` for DM Serif Display, `--font-editorial` for Fraunces, and wire homepage Hero headline/ornament surfaces to the editorial token |
| 2026-03-16 | self | Tried local homepage dogfood with dummy Clerk env values to get past the auth-unavailable middleware block; the page then failed in Clerk runtime with `Publishable key not valid.` | For local homepage/auth dogfood in this repo, use real Clerk test keys; if they are unavailable, record the block explicitly instead of trying placeholder keys |
| 2026-03-13 | self | The execution audit is stricter than the current spec template on terminal-state metadata; I set a `Done` spec to `Successor Slot: 17` and docs audit failed | For any `Done` spec in this repo, always set `Successor Slot: none`; activate the next numbered placeholder in `spec/00_overview.md` and `progress.md` without pointing the closed spec at that number |
| 2026-03-13 | self | Added a gated provider write path but left startup validation and readiness out of sync on encryption prerequisites and mock-mode bypass | For any gated external-provider path, keep startup validation and readiness aligned on the same required secrets plus the same mock-mode exemptions before calling the flow done |
| 2026-03-12 | self | Set an active session spec to `Successor Slot: 09` before handoff, and `pnpm docs:audit -- --sync` correctly failed because active specs must keep `Successor Slot: none` until the session is actually closed | For active specs, keep `Successor Slot: none` throughout implementation; only set the next slot during the final handoff patch that also activates the new placeholder |
| 2026-03-12 | self | Refactored the host create flow but missed source-audit/UAT-style tests and page mocks that encode flow shape (`CREATE_FLOW_TOTAL_STEPS`, review row labels, legacy create-step strings), so the focused suite passed while the full suite still failed | After any flow/step-label refactor, search the whole test tree for old step counts, old label strings, and source-based UAT assertions before trusting targeted passes |
| 2026-03-12 | self | Tried browser dogfood on host create routes without Clerk keys present in local env; middleware returned `503 Authentication unavailable`, so in-app host-flow verification could not proceed | For local browser dogfood of authenticated host/admin routes in this repo, confirm Clerk keys first; if missing, record the environment block explicitly instead of claiming route-level in-app proof |
...
| 2026-03-12 | self | The execution audit only checked structural validity, so a retroactive `Done` plus placeholder could hide a spec that actually stopped at Gate A | Keep explicit terminal metadata in each spec (`Done` vs `Superseded`) and track both `Last Session Spec` and `Last Completed Spec` in `progress.md` so handoff state is machine-checkable |
| 2026-03-12 | self | Kept napkin guidance only under `docs/napkin/SKILL.md`, so repo-local skills and session handoff could bypass it without any enforceable proof | Register a real `.agents/skills/napkin/SKILL.md`, make repo skills load it first, and require `progress.md` `## Napkin Evidence` to link the napkin or say `No durable napkin update.` |
...
| 2026-02-20 | self | In `contribute-details` jsdom tests, I tried `vi.spyOn(navigator, 'maxTouchPoints', 'get')`, but jsdom does not define `maxTouchPoints` by default and the spy throws | For navigator capability fields that may be absent in jsdom, stub via `Object.defineProperty(..., { configurable: true, get: ... })` and restore original descriptors in cleanup |
| 2026-02-20 | self | iOS zoom carry-over fix used unconditional `requestAnimationFrame` before `router.push`, which broke an integration test expecting immediate navigation call timing | Apply deferred navigation only on iOS devices (UA-gated) and keep non-iOS navigation synchronous; when introducing async route timing, proactively update integration assertions |
| 2026-02-19 | self | In host-header hardening tests for OG URL generation, I assumed rejecting an untrusted `x-forwarded-host` should always fall back to `NEXT_PUBLIC_APP_URL`; resolver actually (and correctly) prefers a separately trusted request host/origin (for example `VERCEL_URL`) before config fallback | When validating host poisoning mitigations, assert only that untrusted forwarded host is ignored and that selected origin is from the trusted-host set (request host/origin or configured app URL), not a single fixed fallback |
| 2026-02-19 | self | In `contribute-payment` tests, calling `cleanup()` in `beforeEach` ran unmount logic before `matchMedia` was re-stubbed (after prior `unstubAllGlobals`), causing intermittent `window.matchMedia is not a function` failures in full-suite runs | For jsdom suites that stub browser globals, do `cleanup()` in `afterEach` before `vi.unstubAllGlobals()` so unmount uses the active stubs |
| 2026-02-19 | user | Treated missing step-eyebrow text as a regression after wizard intentionally removed eyebrow indicators across steps | Align wizard UI tests to `WizardStepper` semantics (e.g., progressbar `aria-label`) instead of asserting `WizardEyebrow` copy |
| 2026-02-19 | self | Step 1 mobile refactor removed pre-submit photo-required guard; no-photo submits hit server redirect and dropped unsaved child name/age inputs | Keep client-side photo-required enforcement (`required` when no existing/selected photo or equivalent submit guard) so typed fields are preserved |
